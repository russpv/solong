...
int		render_stats(t_app *a, int b, const char *c, int d);
int		render_sprite(t_app *a, t_img b, int c, int d);
int		render_map(t_app *a);
void	get_sprite(t_app *a, int b, int c);
...
int	render_stats(t_app *app, int stat, const char *pre, int adj)
{
	...
	free((void *)suf);
	result = mlx_string_put(app->mlx_ptr, app->win_ptr, STATSX + adj, STATSY
			+ adj, rgb, (char *)str);
	free((void *)str);
	return (result);
}
...
{
	return (mlx_put_image_to_window(app->mlx_ptr, app->win_ptr, sprite.xpm_ptr,
			col * SCALE, row * SCALE));
}
